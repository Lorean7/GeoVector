...
RouteOptimizer.prototype = {
    ...
    runOneIteration: function() {
        ...
        var routeBest = route;
        var costBest = cost;
        if(route.length > 2){ //Вставил, чтобы не тупило когда один адрес
...
RouteMapper.prototype = {
    ...
    mapRoute: function(routeNum) {
        ...
        ymaps.route(currentRoute.names, {
            ...
            var totalTime = 0;
            var totalLength = 0;
            var startTeg = '<div class="intro-y"><div class="box px-4 py-4 mb-3 flex items-center zoom-in"><div class="w-10 h-10 flex-none image-fit rounded-md overflow-hidden lg:block hidden"></div>'
            var endTeg = '</div></div>'
            for (var k = 1; k < currentRoute.ids.length; k++) {
                ...
                lines.push('<div class="ml-4 mr-auto ml-0"><div class="text-slate-500 text-xs mt-0.5">' + currentRoute['names'][k - 1] + '</div><div class="font-medium">' + currentRoute['names'][k] + '</div></div><div class="py-1 px-2 rounded-full text-xs bg-success text-white cursor-pointer font-medium">' +
                    self.getHumanLength(self.distanceData[key]['length']) + ', ' +
                    self.getHumanTime(self.distanceData[key]['time']) + '</div>');
            ...
            var $total = $('#FulPath');
            $total.html('Полный маршрут: ' + self.getHumanLength(totalLength) + ', ' + self.getHumanTime(totalTime) +' Стомость бензина: '+(totalLength/1000*6.36).toFixed(1) + ' руб.');
            ...
            for (var i = 0; i < lines.length; i++) {
                $list.append(startTeg + lines[i] + endTeg);
            }
